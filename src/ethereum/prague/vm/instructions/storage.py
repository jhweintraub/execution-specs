...
def sreturn_(evm: Evm) -> None:
    """
    Halts execution returning output data directly from storage
    ...
    """
    startingSlot = pop(evm.stack).to_be_bytes32()
    slotCount = pop(evm.stack)
    ...
    for i in range(slotCount):
        ...
        # Output data is the concatenation of continguous transient storage slots
        evm.output += get_storage(evm.env.state, evm.message.current_target, startingSlot + i)
...
def treturn_(evm: Evm) -> None:
    """
    Halts execution returning output data directly from transient storage
    ...
    # GAS
    charge_gas(evm, GAS_WARM_ACCESS * slotCount)

    for i in range(slotCount):
        # OPERATION
        # Output data is the concatenation of continguous transient storage slots
        evm.output += get_transient_storage(
            evm.env.transient_storage, evm.message.current_target, startingSlot + i
        )
